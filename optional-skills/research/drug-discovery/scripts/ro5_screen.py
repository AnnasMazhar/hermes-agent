...
PROPS = (
    "MolecularWeight,XLogP,HBondDonorCount,HBondAcceptorCount,RotatableBondCount,TPSA"
)
...
def check(p):
    mw, logp, hbd, hba, rot, tpsa = (
        float(p.get("MolecularWeight", 0)),
        float(p.get("XLogP", 0)),
        int(p.get("HBondDonorCount", 0)),
        int(p.get("HBondAcceptorCount", 0)),
        int(p.get("RotatableBondCount", 0)),
        float(p.get("TPSA", 0)),
    )
    ...
    return dict(
        ...
        ok=v <= 1 and tpsa <= 140 and rot <= 10,
    )
...
def report(name, r):
    ...
    flags = (f" [Ro5 violations:{r['violations']}]" if not r["ro5"] else "") + (
        " [Veber fail]" if not r["veber"] else ""
    )
    print(
        f"{s}  {name:28s} MW={r['mw']:.0f} LogP={r['logp']:.2f} HBD={r['hbd']} HBA={r['hba']} TPSA={r['tpsa']:.0f} RotB={r['rot']}{flags}"
    )
