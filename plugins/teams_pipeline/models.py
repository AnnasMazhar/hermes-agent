...
class GraphSubscription:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "GraphSubscription":
        return cls(
            subscription_id=str(
                payload.get("subscription_id") or payload.get("id") or ""
            ).strip(),
            resource=str(payload.get("resource") or "").strip(),
            change_type=str(
                payload.get("change_type") or payload.get("changeType") or ""
            ).strip(),
            notification_url=str(
            ...
            latest_renewal_at=payload.get("latest_renewal_at")
            or payload.get("latestRenewalAt"),
...
class TeamsMeetingRef:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "TeamsMeetingRef":
        return cls(
            meeting_id=str(
                payload.get("meeting_id") or payload.get("id") or ""
            ).strip(),
            organizer_user_id=payload.get("organizer_user_id")
            or payload.get("organizerUserId"),
...
class MeetingArtifact:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "MeetingArtifact":
        return cls(
            ...
            artifact_id=str(
                payload.get("artifact_id") or payload.get("id") or ""
            ).strip(),
...
class TeamsMeetingSummaryPayload:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "TeamsMeetingSummaryPayload":
        return cls(
            ...
            key_decisions=list(
                payload.get("key_decisions") or payload.get("keyDecisions") or []
            ),
            action_items=list(
                payload.get("action_items") or payload.get("actionItems") or []
            ),
            risks=list(payload.get("risks") or []),
            call_metrics=dict(
                payload.get("call_metrics") or payload.get("callMetrics") or {}
            ),
            source_artifacts=[
                MeetingArtifact.from_dict(item)
                for item in payload.get("source_artifacts", [])
            ],
            confidence=payload.get("confidence"),
            confidence_notes=payload.get("confidence_notes")
            or payload.get("confidenceNotes"),
...
class TeamsMeetingPipelineJob:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "TeamsMeetingPipelineJob":
        ...
        summary_payload = payload.get("summary_payload") or payload.get(
            "summaryPayload"
        )
        return cls(
            job_id=str(payload.get("job_id") or payload.get("jobId") or "").strip(),
            event_id=str(
                payload.get("event_id") or payload.get("eventId") or ""
            ).strip(),
            source_event_type=str(
                payload.get("source_event_type") or payload.get("sourceEventType") or ""
            ).strip(),
            dedupe_key=str(
                payload.get("dedupe_key") or payload.get("dedupeKey") or ""
            ).strip(),
            ...
            meeting_ref=TeamsMeetingRef.from_dict(meeting_ref_payload)
            if meeting_ref_payload
            else None,
            ...
            error_info=dict(
                payload.get("error_info") or payload.get("errorInfo") or {}
            ),
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        return _clean_dict({
            ...
            "summary_payload": self.summary_payload.to_dict()
