...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _submit_on_loop(self, loop: Any, coro: Any) -> bool:
        ...
        try:
            ...
            coro.close()
            logger.warning(
                "[Feishu] Failed to schedule background callback work", exc_info=True
            )
            return False
        future.add_done_callback(self._log_background_failure)
        return True
    ...
    def _handle_approval_card_action(
        ...
        if not self._submit_on_loop(
            loop, self._resolve_approval(approval_id, choice, user_name)
        ):
            return (
                P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
            )
    ...
    async def _resolve_update_prompt(
        self, prompt_id: Any, answer: str, user_name: str
    ) -> None:
